            ...
            For i = 0 To nRet - 1
               If shi(i).ShareType = STYPE_DISKTREE Then
                  If InStr(1, FileSpec, shi(i).Path, vbTextCompare) = 1 Then
                     ' This element starts with the same path so we
                     ' have to accept first match. Note that if the
                     ' share is a folder (not the root) the filespec
                     ' starts one character further in the buffer.
                     GetUncName = "\\" & CurrentMachineName & _
                                  "\" & shi(i).Netname & _
                                  Mid(FileSpec, Len(shi(i).Path) + _
                                  Abs(Len(shi(i).Path) > 3))
                     Exit For
                  End If
               End If
            Next i
...
Private Function GetConnection95(ByVal LocalName As String) As String
   Dim nRet As Long
   Dim Buffer As String
   
   ' Allocate buffer, and call
   Buffer = Space(MAX_PATH)
   nRet = WNetGetConnection(LocalName, ByVal Buffer, Len(Buffer))
   Debug.Print nRet
   GetConnection95 = TrimNull(Buffer)
End Function
